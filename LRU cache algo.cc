...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
  {
    ...
    {
      // Move the accessed cache line to the most recently used position
      std::rotate(tags + idx * ways, tags + idx * ways + i, tags + idx * ways + i + 1);
      return &tags[idx*ways];
    }
  }
...
{
  size_t idx = (addr >> idx_shift) & (sets - 1);
  size_t way = ways - 1; // Select the least recently used cache line (last way)
  uint64_t victim = tags[idx * ways + way];

  // Move all cache lines in the set to the left (least recently used position)
  std::rotate(tags + idx * ways, tags + idx * ways + way, tags + idx * ways + way + 1);
